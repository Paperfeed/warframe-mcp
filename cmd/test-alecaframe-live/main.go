...
func main() {
	...
	if userHash == "" || publicToken == "" {
		fmt.Println("Error: ALECAFRAME_USER_HASH and ALECAFRAME_PUBLIC_TOKEN environment variables must be set")
		os.Exit(1)
	}

	fmt.Printf("Testing Alecaframe API with credentials...\n")
	fmt.Printf("User Hash: %s...\n", maskString(userHash, 8))
	fmt.Printf("Public Token: %s...\n\n", maskString(publicToken, 8))

	client := alecaframe.NewClient(userHash, publicToken)
